 ...
 * @package App
 * @version 1.0
 * @version 1.0.1 Jan. 2018 - Définition du mode d'exécution de l'application
**/
namespace App;
...
class appLoader {
	...
	public function __construct($appRootPath){
		...
		// Détermine le mode d'exécution de l'application
		self::$wp->runMode($this->_defineRunMode());
		
		if(self::$wp->runMode() == "dev"){
	...
	private function readConfiguration(){ 
		if(file_exists($this->toRoot . "_commun/configs/appConfigure.json")){
			...
			if(is_null($this->appConfig)){
				...
				$message = "Le fichier de configuration : appConfigure.json est incorrect.<br />\nVérifiez la syntaxe JSON.";
			...
			// Problème de lecture des configurations de l'application
			$error = new error();
			
			$message = "Le fichier de configuration : _commun/configs/appConfigure.json n'a pas été trouvé.<br />\nVérifiez votre arborescence.";
	 ...
	 * @return string
	 */
	private function _defineRunMode(){
		$runMode = "dev";
		
		$currentServerName = $_SERVER["SERVER_NAME"];
		
		if(property_exists($this->appConfig, "domains")){
			foreach($this->appConfig->domains as $domain){
				if($domain->name == $currentServerName){
					$runMode = "prod";
					break;
				}
			}
		} else {
			// La donnée n'a pas été déterminée
			if($currentServerName != "localhost" 
					|| $currentServerName != "127.0.0.1" 
					|| substr($currentServerName, -3) != "dev"
					|| substr($currentServerName, -3) != "wrk"){
				$runMode = "prod";
			}
		}
